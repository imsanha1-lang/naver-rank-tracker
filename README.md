...
**매장 1개에 키워드 여러 개**를 묶는 형식입니다.
```json
[
  { "place_name": "OOO헤어", "keywords": ["강남역 미용실", "강남 미용실", "역삼동 미용실"] },
  { "place_name": "OO커피",  "keywords": ["성수동 카페", "성수 디저트"] }
]
```
> 대시보드 하단 "매장 관리"에서 (키워드+매장) 쌍을 추가하고 **[JSON 복사]** 한 뒤 이 파일에 붙여넣어도 됩니다.
> 같은 키워드는 하루에 한 번만 검색하므로, 매장이 많아도 검색 횟수 = **고유 키워드 수** 입니다.
